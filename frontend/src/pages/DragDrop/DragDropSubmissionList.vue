<template>
		...
		<ListView :columns="columns" :rows="submissions.data" row-key="name" :options="{ showTooltip: false, selectable: false }">
			...
			<ListRows>
				<router-link
					...
					:to="{ name: 'DragDropSubmission', params: { submission: row.name } }"
				>
					<ListRow :row="row" />
				</router-link>
			</ListRows>
	...
	<EmptyState v-else type="Drag & Drop Submissions" />
</template>
<script setup>
import {
	Breadcrumbs,
...
const breadcrumbs = computed(() => [{ label: __('Drag & Drop Submissions') }])

usePageMeta(() => ({
	title: __('Drag & Drop Submissions'),
	icon: brand.favicon,
}))
</script>
