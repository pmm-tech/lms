<template>
	<header class="sticky top-0 z-10 flex items-center justify-between border-b bg-surface-white px-3 py-2.5 sm:px-5">
		...
		<div v-if="!readOnlyMode" class="flex items-center space-x-2">
			<router-link v-if="details.doc?.name" :to="{ name: 'WordHuntPage', params: { activityID: details.doc.name } }">
				<Button>{{ __('Test Activity') }}</Button>
			</router-link>
			<router-link
				v-if="details.doc?.name"
				:to="{ name: 'WordHuntSubmissionList', params: { activityID: details.doc.name } }"
			>
	...
	<div v-if="details.doc" class="py-5">
		...
		<div class="px-20 pb-5 space-y-5 border-b mb-5">
			<div class="text-lg text-ink-gray-9 font-semibold mb-4">{{ __('Passage') }}</div>
			<div class="rounded-lg border bg-surface-gray-1 p-4 text-sm text-ink-gray-7">
		...
		<div class="px-20 pb-5 space-y-5 mb-5">
			<div class="flex items-center justify-between mb-4">
				<div class="text-lg font-semibold text-ink-gray-9">{{ __('Blanks') }}</div>
			...
			<div class="space-y-4">
				<div v-for="(item, idx) in details.doc.items" :key="item.name || idx" class="rounded-lg border p-4 space-y-4">
					<div class="grid grid-cols-3 gap-4">
						<div class="rounded-lg border bg-surface-gray-1 px-3 py-2 text-sm text-ink-gray-7">
...
const breadcrumbs = computed(() => [
	...
	{ label: details.doc?.title, route: { name: 'WordHuntForm', params: { activityID: props.activityID } } },
])
