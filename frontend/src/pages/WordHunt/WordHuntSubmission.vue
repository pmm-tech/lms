	...
	<div v-if="details.doc" class="w-2/3 border-x mx-auto py-5">
		...
		<div class="space-y-4 border-b pb-5 px-10">
			<div class="grid grid-cols-2 gap-5">
				<FormControl
					v-model="details.doc.activity_title"
					:label="__('Activity')"
					:disabled="true"
				/>
				<FormControl
					v-model="details.doc.member_name"
					:label="__('Member')"
					:disabled="true"
				/>
			</div>
			<div class="grid grid-cols-2 gap-5">
				<FormControl
					v-model="details.doc.score"
					:label="__('Score')"
					:disabled="true"
				/>
				<FormControl
					v-model="details.doc.percentage"
					:label="__('Percentage')"
					:disabled="true"
				/>
			</div>
		...
		<div class="divide-y">
			<div v-for="row in details.doc.result" class="py-5 px-10 space-y-4">
				<div class="text-ink-gray-9">
					<span class="font-semibold">{{ __('Blank') }}: </span>
					<span>{{ row.idx || '-' }}</span>
				</div>
				<div class="grid grid-cols-2 gap-5">
					<FormControl
						v-model="row.submitted_answer"
						:label="__('Submitted Answer')"
						:disabled="true"
					/>
					<FormControl
...
const breadcrumbs = computed(() => [
	{
		label: __('Word Hunt Submissions'),
		route: {
			name: 'WordHuntSubmissionList',
			params: { activityID: details.doc.activity },
		},
	},
