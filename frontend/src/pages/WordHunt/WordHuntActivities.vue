<template>
	...
	<div class="py-5 mx-5">
				...
				<ListRow v-for="row in activities.data" :row="row" class="hover:bg-surface-gray-1">
					<template #default="{ column }">
						<ListRowItem :item="row[column.key]" :align="column.align">
							...
							<div v-else-if="column.key === 'actions'" class="flex items-center justify-end gap-2">
								<Button
									v-if="!readOnlyMode"
									size="sm"
									variant="ghost"
								...
								<Button size="sm" variant="ghost" @click.stop="openSubmissions(row.name)">
									{{ __('Submissions') }}
								</Button>
							</div>
							<div v-else>
		...
		<div v-else-if="hasActiveSearch" class="rounded border bg-surface-white px-6 py-12 text-center">
		...
		<div v-if="activities.data?.length" class="mt-4 flex items-center justify-between gap-3">
			<div class="text-sm text-ink-gray-5">
				{{ pageRangeLabel }}
			</div>
			<div class="flex items-center gap-2">
				<Button variant="ghost" :disabled="currentPage === 1 || activities.loading" @click="goToPreviousPage">
					{{ __('Previous') }}
				</Button>
				<div class="text-sm text-ink-gray-7">
					{{ __('Page {0} of {1}').format(currentPage, totalPages) }}
				</div>
				<Button variant="ghost" :disabled="!activities.hasNextPage || activities.loading" @click="goToNextPage">
					{{ __('Next') }}
				</Button>
			</div>
		</div>
...
const breadcrumbs = computed(() => [{ label: __('Word Hunt Activities'), route: { name: 'WordHuntActivities' } }])
